# Source: https://tari.com/updates/2022-09-22-update-89.html
...
[< Back to updates](https://tari.com/updates)![Pivoting on the DAN](https://tari.com/assets/updates/img/update-background.jpg)

# Pivoting on the DAN

September 22, 2022

I came across an article a few months ago by Dan Hughes, founder of a DeFi-focused proof-of-stake project, Radix, that opines that [blockchains can't scale](https://assets-global.website-files.com/6053f7fca5bf627283b582c2/60c3808d955fe9316ac6af05_3.%20Why%20Blockchains%20Can%27t%20Scale_compressed%20(2).pdf). It makes many points (all of them correct), such as
...
We've discussed this very point at length within the Tari community and take it as near-axiomatic that [scaling a level 1 blockchain network is not feasible](https://rfc.tari.com/RFC-0001_overview.html?highlight=scal#currency-tokens-and-digital-assets).

Our intention was always to build a base layer proof-of-work based network that prioritises decentralisation and security over speed and latency and _supplement_ this with one or more second layer networks than emphasise scalability and speed. Notice that we explicitly pick 2 different goals on two different layers to get everything we want.
...
The obvious "solution" here is to run independent side-chains, so that there _is no global state_! It's a cop-out, but it kinda makes sense, right?
...
We've been mulling over this problem for a long time and [RFC-312](https://rfc.tari.com/RFC-0312_DANHighLevelSpecification.html#inter-contract-interactions) discusses several strategies that Tari side-chains could use to play nicely together. Admittedly, all of them pose significant technological hurdles.

The most common strategy is to use bridge contracts to allow side-chains to communicate with each other. And there's no denying the fact: Building secure bridges is hard. To whit, [hack1](https://zerion.io/blog/nomad-bridge-hack/), [hack2](https://halborn.com/explained-the-ronin-hack-march-2022/), [hack3](https://halborn.com/explained-the-qubit-hack-january-2022/), and [hack4](https://halborn.com/explained-the-wormhole-hack-february-2022/), in 2022 alone.

## Why bring all of this up?

It turns out that Dan wasn't just being a typical crypto-twitter negative nellie. He has actually developed a little-known consensus algorithm called [Cerberus](https://arxiv.org/pdf/2008.04450v1.pdf) that he believes solves the scalability problem once and for all.
...
Instead of VNs managing state for a _given_ contract, VNs manage predetermined pieces of state, out of the _entire universe of possible states_! This mapping is essentially random, so that state management is automatically load-balanced across all nodes in the network. Furthermore, as nodes join the network, the portion of the overall state space a VN is responsible for shrinks accordingly, meaning that capacity essentially scales with the number of nodes.
...
This is the secret sauce of Cerberus, and when it clicks for you (you may have to read the [Cerberus](https://arxiv.org/pdf/2008.04450v1.pdf) paper or read Radix's [Infographic series](https://www.radixdlt.com/post/cerberus-infographic-series-chapter-i)), you'll understand why it's so exciting.
...
It should then be fairly obvious that if VNs are reaching consensus on small bits of state rather than information related to specific contracts, inter-contract interactions _just work (TM)_.
...
There will be a community discussion on this topic on the [Tari Discord on 2022-09-26 at 09:00 UTC](https://discord.com/channels/892738558316662855/893157378789351475/1022072734399868979). So make sure you don't miss it!
