...
In addition, transaction input detection has seen some work that addresses existing issues in applications that depend on it, like Tari Swarm. In particular, the wallet will now ensure that all referenced substates in a transaction are detected, which it checks by calling the indexer directly. Further development will be required to refine this process, but the fix has already improved the overall accuracy of transactions being handled.
